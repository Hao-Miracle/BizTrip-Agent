...
`web` 会启动只绑定本机的页面。账号只需要配置一次；之后每次扫描只选择日期范围并生成报销包。系统会按邮箱地址自动选择 IMAP 服务器。
...
默认会输出到 `output/` 并生成审阅页面。日期范围留空时扫描最近 60 封邮件；需要调整数量、输出目录或规则模式时，展开“更多扫描选项”；需要 LLM 或手动 IMAP 时，展开“高级配置”。
