...
默认会输出到 `output/` 并生成审阅页面。报销期间决定结果边界；需要调整扫描邮件数量、输出目录或规则模式时，展开“高级扫描选项”。普通使用不需要 LLM；想增强复杂邮件识别时，展开“高级配置”填写 LLM Base URL 和 API Key。配置后不需要进入对话窗口，点击“开始生成”时系统会自动调用 LLM 辅助分类、提取和行程聚合。
